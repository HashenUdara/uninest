...
<layout:admin-dashboard pageTitle="Communitys" activePage="communitys">
    <header class="c-page__header">
      <nav class="c-breadcrumbs" aria-label="Breadcrumb">
        <a href="${pageContext.request.contextPath}/admin/dashboard">Admin</a>
        <span class="c-breadcrumbs__sep">/</span>
        <span aria-current="page">Communitys</span>
      </nav>
      <div class="c-page__titlebar">
        <div>
          <h1 class="c-page__title">Communitys</h1>
          <p class="c-page__subtitle u-text-muted">
            <c:choose>
              <c:when test="${currentStatus == 'pending'}">Review and manage community requests.</c:when>
              <c:when test="${currentStatus == 'approved'}">Approved communitys.</c:when>
              <c:when test="${currentStatus == 'rejected'}">Rejected communitys.</c:when>
            </c:choose>
          </p>
        </div>
        <a class="c-btn" href="${pageContext.request.contextPath}/admin/communitys/create">
          <i data-lucide="plus"></i> Create Community
        </a>
      </div>
      <div class="c-tabs" role="tablist" aria-label="Community tabs">
        <a href="${pageContext.request.contextPath}/admin/communitys?status=pending" 
           class="c-tabs__link ${currentStatus == 'pending' ? 'is-active' : ''}" 
           role="tab" 
           aria-selected="${currentStatus == 'pending' ? 'true' : 'false'}">Pending</a>
        <a href="${pageContext.request.contextPath}/admin/communitys?status=approved" 
           class="c-tabs__link ${currentStatus == 'approved' ? 'is-active' : ''}" 
           role="tab" 
           aria-selected="${currentStatus == 'approved' ? 'true' : 'false'}">Accepted</a>
        <a href="${pageContext.request.contextPath}/admin/communitys?status=rejected" 
      ...
      <div class="c-table-toolbar">
        <div class="c-table-toolbar__left">
          <span class="u-text-muted">
            <span class="js-comm-count">${communitys.size()}</span> communitys
          </span>
        </div>
      </div>
      <div class="c-table-wrap">
        <table class="c-table c-table--sticky" aria-label="${currentStatus} communitys" data-comm-table>
          <thead>
            <tr>
              <th>Community</th>
              <th>Description</th>
          ...
          <tbody>
            <c:forEach items="${communitys}" var="o">
              <tr>
                <td>
                ...
                <td class="u-text-right">
                  <div class="c-table-actions">
                    <c:if test="${currentStatus == 'pending'}">
                      <form method="post" action="${pageContext.request.contextPath}/admin/communitys/approve" style="display:inline" class="js-comm-approve">
                        <input type="hidden" name="id" value="${o.id}" />
                        <button class="c-btn c-btn--sm" type="submit" aria-label="Approve">Approve</button>
                      </form>
                      <form method="post" action="${pageContext.request.contextPath}/admin/communitys/reject" style="display:inline" class="js-comm-reject">
                        <input type="hidden" name="id" value="${o.id}" />
                    ...
                    <a class="c-icon-btn" aria-label="Edit" href="${pageContext.request.contextPath}/admin/communitys/edit?id=${o.id}">
                      <i data-lucide="pencil"></i>
                    </a>
                    <form method="post" action="${pageContext.request.contextPath}/admin/communitys/delete" style="display:inline" class="js-comm-delete">
                      <input type="hidden" name="id" value="${o.id}" />
